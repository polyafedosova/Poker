...
public class TwoPairCombinationService implements ICombinationService {
    @Override
    public List<Card> calc(List<Card> unionCard) {
        ...
        if (countPair >= 2) {
            if (countPair == 3) {
                int indexMinRank = Math.min(Math.min(combination.get(0).getRank().ordinal(), combination.get(2).getRank().ordinal()),
                        combination.get(4).getRank().ordinal());
